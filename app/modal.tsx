...
export default function Modal() {
  ...
  const updateThreadText = (id: string, text: string) => {
    setThreads((prevThreads) =>
      prevThreads.map((prevThread) =>
        prevThread.id === id ? { ...prevThread, text } : prevThread
      )
    );
  };
  ...
  const removeThread = (id: string) => {
    setThreads((prevThreads) =>
      prevThreads.filter((prevThread) => prevThread.id !== id)
    );
  };
      ...
      setThreads((prevThreads) =>
        prevThreads.map((prevThread) =>
          prevThread.id === id
            ? {
                ...prevThread,
                imageUris: prevThread.imageUris.concat(
  ...
  const takePhoto = async (id: string) => {
    ...
    if (mediaStatus === "granted" && !!result.assets?.[0].uri) {
    ...
    setThreads((prevThreads) =>
      prevThreads.map((prevThread) =>
        prevThread.id === id
          ? {
              ...prevThread,
              imageUris: prevThread.imageUris.filter(
                (uri) => uri !== uriToRemove
              ),
            }
          : prevThread
...
export function ListFooter({
  ...
  return (
    <View style={styles.listFooter}>
      ...
      <View>
        <Pressable style={styles.input} onPress={addThread}>
